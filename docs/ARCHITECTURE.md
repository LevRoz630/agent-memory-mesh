...
is filtered to rows whose `owner` is a roster address (`runQuery` in `src/arkiv.mjs`). A stranger's
wallet can write a hydra-shaped heartbeat, claim or `done`, and no agent will ever see it.
...
`reopened` verdict is recorded; nothing re-opens the work yet.
